    ...
    <div class="login-box" >
      <div style="padding: 50px 30px;background-color:rgba(143,233,244,50%);border-radius: 8px;box-shadow: 0 0 10px rgba(0,0,0,0.2)">
        <el-form ref="formRef" :rules="data.rules" :model="data.form"  style="width: 330px">
          <div style="margin-bottom: 40px;color:#d7f6ea;font-size: 20px;font-weight: bold;text-align: center;">欢迎注册</div>
          ...
          <el-form-item prop="confirmpassword">
            <el-input show-password size="large" v-model="data.form.confirmpassword" :prefix-icon="Lock" placeholder="请确认密码"  class="input-with-icon" ></el-input>
          </el-form-item >
          ...
          <div style="text-align: right;color: #2c2c31">已有账号?请<a style="color: #1756ce;text-decoration:none" href="/register">登录</a></div>
        </el-form>
      </div>
    </div>
  </div>
...
import right from "@element-plus/icons/lib/Right.js";
  ...
  formRef.value.validate((valid) =>{
        if(valid){
          request.post('/register',data.form).then(res=>{
            if(res.code === '200'){
              localStorage.setItem('admin-info',JSON.stringify(res.data))//转换为JSON字符串
